...
public class HandMirrorReflection : MonoBehaviour
{
    ...
    private void RenderCamera(Camera cam, Renderer rend, Camera.StereoscopicEye eye, ref RenderTexture reflectionTexture)
    {
        ...
        if (cam.stereoEnabled)
        {
            ...
            if (eye == Camera.StereoscopicEye.Left)
            {
                ...
                Vector3 position;
                device.TryGetFeatureValue(CommonUsages.leftEyePosition, out position);
                ...
                XRNodeState leftNodeState = nodeStates.Where(nodeState => nodeState.nodeType == XRNode.LeftEye).First();
                */
            }
            else
            {
